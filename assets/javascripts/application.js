...
var searchPlugin = function searchPlugin() {
  ...
  var LunrSearch = function () {
    function LunrSearch(elem, options) {
      ...
      _classCallCheck(this, LunrSearch);
      ...
      this.loadIndexData(function (data) {
        ...
        _this.searchWorker.postMessage(data);
      });
      ...
      this.searchWorker.addEventListener('message', function (event) {
        if (event.data.type.indexed) {
          _this.populateSearchFromQuery();
        } else {
          if (event.data.type.quicksearch) {
            _this.displayQuicksearch(event.data.query, event.data.results);
          } else {
            _this.displaySearchResults(event.data.query, event.data.results);
          }
        }
      });
      this.searchMoreButton.on('click', function (event) {
        if (_this.page) {
    ...
    _createClass(LunrSearch, [{
      ...
      key: 'loadIndexData',
      value: function loadIndexData(callback) {
        $.getJSON(this.indexDataUrl, callback);
      }
    }, {
      key: 'bindQuicksearchKeypress',
      value: function bindQuicksearchKeypress() {
        var _this2 = this;

        var oldValue = this.search_elem.val();
        this.search_elem.bind('keyup', debounce(function () {
          var newValue = _this2.search_elem.val();
          _this2.search(newValue, newValue === oldValue ? null : true);
          oldValue = newValue;
        }));
      }
    }, {
      key: 'bindQuicksearchFocus',
      value: function bindQuicksearchFocus() {
        var _this3 = this;

        this.search_elem.bind('focus', debounce(function () {
          if (_this3.search_elem.val()) {
            _this3.quickSearchResults.show();
          }
        }));
      }
    }, {
      key: 'bindQuicksearchBlur',
      value: function bindQuicksearchBlur() {
        var _this4 = this;

        this.search_elem.bind('blur', debounce(function () {
          _this4.quickSearchResults.hide();
        }));
      }
    }, {
      key: 'bindQuicksearchMousedown',
      value: function bindQuicksearchMousedown() {
      ...
      key: 'search',
      value: function search(query, quickSearch) {
      ...
      value: function displayQuicksearch(query, entries) {
        ...
        if (entries.length > 0) {
          ...
          var _href = $('.quicksearch-seemore').attr('href');
      ...
      value: function displaySearchResults(query, entries) {
      ...
      key: 'populateEntries',
      value: function populateEntries() {
        var max = 50 * (this.page || 1);
        var entriesToShow = this.entries.slice(max - 50, max - 1);
      ...
      value: function populateSearchFromQuery() {
  ...
  $.fn.lunrSearch = function (_options) {
    var options = $.extend({}, $.fn.lunrSearch.defaults, _options);
